...
def load_data(vintage, q_var):
    ...
    # - Quarterly data --------------------------------------------------------------
    # 1. Download data
    orig_q = (pd.read_csv(f'../../data/vintages_quarterly_update/{vintage}.csv')
                .dropna(how='all'))
...
def load_text_data(vintage, q_var, text_type="topics", estimation_period="2007", num_topics="200", source="all", selected=""):
    ...
    # 1. Download data
    filename = f'../../data/vintages_monthly_{q_var}_{text_type}_{estimation_period}_{num_topics}_{source}{selected}_update/{vintage}.csv'
...
def get_forecasts(forecast_month, q_var, additional_factors, factor_multiplicities, factor_orders, start, text_type="topics",
    ...
    defn_q = pd.read_excel('../../data/data_quarterly_update/variables_definitions.xlsx')
    defn_q = defn_q[defn_q.Mnemonic == q_var]
    defn_q.index = defn_q['Mnemonic']
    if with_text:
        # Load the definitions Excel file for text variables
        defn_text = pd.read_excel(f'../../data/data_text/variables_definitions_{q_var}_{text_type}_{estimation_period}_{num_topics}_{source}{selected}_update.xlsx')
