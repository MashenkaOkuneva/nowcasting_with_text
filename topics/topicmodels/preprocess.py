...
# import data for pre-processing  (contains stopwords, names, and collocations)
...
class RawDocs():
    ...
    def __init__(self, doc_data, contraction_split=False, collocations = True):
        ...
        collocations: whether to combine collocations into one token (e.g., IG Metall => IG_Metall)
        """

        if isinstance(doc_data, str):
            try:
                with codecs.open(doc_data, 'r', 'utf-8-sig') as f:
                    raw = f.read()
...
must be passed to RawDocs")
        ...
        if collocations:
            for k, v in preprocess_data.dic_trigrams.iteritems():
                self.docs = map(lambda x: re.sub(k, v, x), self.docs)   
            for k, v in preprocess_data.dic_bigrams.iteritems():
                self.docs = map(lambda x: re.sub(k, v, x), self.docs)  

        self.docs = [s.lower() for s in self.docs]

        self.stopwords = preprocess_data.stp
        
        self.names = preprocess_data.names
    ...
    def token_clean(self, length, numbers=True):
        ...
        numbers: strip out non-alpha tokens, but keep collocations with the underscore.
        """

        def clean1(tokens):
            return [t for t in tokens if (t.isalpha() == 1 and len(t) > length) or (u'_' in t and len(t) > length)] 
        def clean2(tokens):
            return [t for t in tokens if (t.isalnum() == 1 and len(t) > length) or (u'_' in t and len(t) > length)]
    ...
    def stem(self):
        ...
        def s(tokens):
            return [stemmer.stem(t) if u'_' not in t else t for t in tokens]
    ...
    def words_remove(self, dictionary):
        ...
        def remove_words(tokens):
            return [t for t in tokens if t not in dictionary]

        self.tokens = list(map(remove_words, self.tokens)) 
    ...
    def term_rank(self, items, print_output=True):
        ...
        if print_output:
            with codecs.open('df_ranking.csv', 'w', 'utf-8-sig') as f:
                    for p in self.df_ranking:
                        f.write("%s,%d\n" % (p[0], p[1]))
            with codecs.open('tfidf_ranking.csv', 'w', 'utf-8-sig') as f:
