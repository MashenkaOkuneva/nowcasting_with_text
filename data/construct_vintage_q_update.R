...
series_codes <- c("BBKRT.Q.DE.Y.A.AG1.CA010.A.I", # Gross Domestic Product (GDP)
                  "BBKRT.Q.DE.Y.A.CA1.BA100.V.A", # Private Consumption Expenditure (nominal)
                  "BBKRT.Q.DE.Y.A.CE1.BAA00.A.I"  # Gross Capital Investment (private sector, without buildings)
)
...
# Ensure the "data_quarterly_update" directory exists; if not, create it
if(!dir.exists("data_quarterly_update")){
  dir.create("data_quarterly_update")
}
...
for (i in seq_along(series_codes)) {
  ...
  save(list = short_name, file = file.path("data_quarterly_update", paste0(short_name, ".Rda")))
}
...
# Write to CSV
file_name <- paste0(file_vintage, "_update",".csv")
